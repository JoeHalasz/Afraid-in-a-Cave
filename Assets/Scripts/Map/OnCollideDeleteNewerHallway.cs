...
public class OnCollideDeleteNewerHallway : MonoBehaviour
{
    ...
    void OnTriggerStay(Collider collider)
    {
        ...
        if (collider.gameObject.name.Contains("Hallway") || collider.gameObject.name.Contains("Room"))
        {
            // remove the "Hallway" or "Room" from the name to get the number
            string thisName = gameObject.name.Replace("Hallway", "").Replace("Room", "").Replace("(Clone)","");
            string otherName = collider.gameObject.name.Replace("Hallway", "").Replace("Room", "").Replace("(Clone)","");
            ...
            if (thisNumber > otherNumber)
            {
                ...
                {
                    if (child.name.Contains("Hallway") || child.name.Contains("Room"))
                    {
                        Debug.LogWarning($"Child {child.name} of {gameObject.name} has the word 'Hallway' or 'Room' in its name. This may cause issues.");
                    }
                }
                Destroy(gameObject);
            }
            else if (thisNumber < otherNumber)
            {
                ...
                foreach (Transform child in collider.transform)
                {
                    if (child.name.Contains("Hallway") || child.name.Contains("Room"))
                    {
                        Debug.LogWarning($"Child {child.name} of {collider.gameObject.name} has the word 'Hallway' or 'Room' in its name. This may cause issues.");
                    }
                }
